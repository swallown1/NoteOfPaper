...
class MF(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            # Input data.
            self.user = tf.keras.backend.placeholder(tf.int32, shape=[None])  # None
            self.item_pos = tf.keras.backend.placeholder(tf.int32, shape=[None])  # None * 1
            self.item_neg = tf.keras.backend.placeholder(tf.int32, shape=[None])
            self.dropout_keep = tf.keras.backend.placeholder(tf.float32, shape=[None])
            self.train_phase = tf.keras.backend.placeholder(tf.bool)
            self.ru = tf.keras.backend.placeholder(tf.int32, shape=[None, None])
            self.cnt = tf.keras.backend.placeholder(tf.float32, shape=[None])
            # Variables.
            self.weights = self._initialize_weights()
            self.alpha = tf.keras.backend.placeholder(tf.float32, shape=[None])
